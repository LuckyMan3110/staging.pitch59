...
} from '../../utility-functions/exif.utils';
import { resizeCanvas } from '../../utility-functions/resize.utils';
...
export class ImageCropperComponent implements OnChanges {
      ...
      if (this.isValidImageType(imageType)) {
        resetExifOrientation(event.target.result)
          .then((resultBase64: string) =>
            this.loadBase64Image(resultBase64)
          )
  ...
  private setCropperScaledMinWidth(): void {
    if (this.maintainAspectRatio) {
      this.cropperScaledMinWidth = Math.max(
        20,
        this.cropperScaledMinHeight * this.aspectRatio
      );
    } else if (this.cropperMinWidth > 0) {
      this.cropperScaledMinWidth = Math.max(
        20,
    ...
    if (this.sourceImage.nativeElement && this.originalImage != null) {
      ...
      if (ctx) {
        ...
        if (this.resizeToWidth) {
          resizeRatio = this.getResizeRatio(height, true);
        } else if (this.resizeToHeight) {
          resizeRatio = this.getResizeRatio(height, false);
        }
        if (resizeRatio !== 1) {
  ...
  private getResizeRatio(dimention: number, isWidth: boolean): number {
    if (isWidth) {
      return this.resizeToWidth > 0 &&
      (!this.onlyScaleDown || dimention > this.resizeToWidth)
        ? this.resizeToWidth / dimention
        : 1;
    } else {
      return this.resizeToHeight > 0 &&
      (!this.onlyScaleDown || dimention > this.resizeToHeight)
        ? this.resizeToHeight / dimention
        : 1;
    }
  }
